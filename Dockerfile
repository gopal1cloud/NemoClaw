...
# commands are done, so runtime Discord/WebSocket egress uses the OpenShell
# gateway proxy without forcing image-build npm traffic through that proxy.
# Keep Gateway loopback control-plane traffic on OpenClaw's narrow direct bypass;
# routing 127.0.0.1:18789 through OpenShell is blocked as loopback SSRF.
RUN python3 -c "\
import json, os; \
