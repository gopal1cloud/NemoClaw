...
//
// Failure handling:
// - `stat` failures are recorded as issues so the caller cannot mistake
//   an unreachable sandbox for a clean lockdown.
// - `lsattr` failures are recorded as issues only when `verifyChattr` is
//   true. Some images do not ship `lsattr`, so when chattr verification
//   was not requested the binary's absence is treated as an unavailable
//   check rather than a tamper signal.
