...
`,
  ...
  it("emits a single fast-reentry marker for two concurrent fresh late scope upgrades", () => {
    // Two CLI clients arrive in the same pending wave after the watcher
    // converged to slow-mode keepalive. This stands in for the cascading
    // late-upgrade case that surfaces when two sibling sandboxes each
    // trigger their own scope upgrade through the gateway at roughly the
    // same time: each sandbox runs its own watcher, but the property the
    // override protects (rising-edge bump per fresh requestId, one marker
    // per poll wave, no per-id duplicate bumps) is the same in either
    // shape. Both upgrades must be approved and the watcher must emit
    // exactly one slow-mode marker for the wave.
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "nemoclaw-auto-pair-concurrent-"));
    const fakeOpenclaw = path.join(tmpDir, "openclaw");
    const stateFile = path.join(tmpDir, "list-count");
    const approveLog = path.join(tmpDir, "approvals.log");
    const browserClient = { clientId: "openclaw-control-ui", clientMode: "webchat" };
    const cliA = { clientId: "openclaw-cli", clientMode: "cli" };
    const cliB = { clientId: "openclaw-cli", clientMode: "cli" };
    const initial = JSON.stringify({
      pending: [{ requestId: "browser-pair", ...browserClient }],
      paired: [],
    });
    const browserPaired = JSON.stringify({ pending: [], paired: [browserClient] });
    const twoLate = JSON.stringify({
      pending: [
        { requestId: "late-cli-a", ...cliA },
        { requestId: "late-cli-b", ...cliB },
      ],
      paired: [browserClient],
    });
    const allPaired = JSON.stringify({ pending: [], paired: [browserClient, cliA, cliB] });
...
`,
    ...
    try {
      ...
      expect(run.stdout).toContain(
        "[auto-pair] approved request=late-cli-a client=openclaw-cli mode=cli",
      );
      expect(run.stdout).toContain(
        "[auto-pair] approved request=late-cli-b client=openclaw-cli mode=cli",
      );
      // One marker per wave: the two-wide late upgrade emits a single
      // slow-mode `fast-reentry bumped` marker that records APPROVED=3
      // (browser + both late upgrades).
  ...
  it("bumps the fast-reentry counter at most once for a sticky failing allowlisted approval", () => {
    // A late CLI scope upgrade whose approve call keeps failing must bump
    // the fast-reentry override exactly once on the rising edge and never
    // again, so a single broken upgrade cannot keep the watcher in 1s
    // polling for the rest of the deadline window. The override drains
    // after FAST_REENTRY_POLLS sleeps and the watcher reverts to the
    // configured slow cadence, leaving room for genuinely fresh requestIds
    // to bump again. The approve call is retried each poll (sticky-cli is
    // intentionally not added to HANDLED on a non-zero approve) but only
    // the first attempt arms the override.
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "nemoclaw-auto-pair-sticky-"));
    const fakeOpenclaw = path.join(tmpDir, "openclaw");
    const stateFile = path.join(tmpDir, "list-count");
    const approveLog = path.join(tmpDir, "approve-attempts.log");
    const browserClient = { clientId: "openclaw-control-ui", clientMode: "webchat" };
    const cliClient = { clientId: "openclaw-cli", clientMode: "cli" };
    const initial = JSON.stringify({
      pending: [{ requestId: "browser-pair", ...browserClient }],
    ...
    const browserPaired = JSON.stringify({ pending: [], paired: [browserClient] });
    const stickyLate = JSON.stringify({
      pending: [{ requestId: "sticky-cli", ...cliClient }],
      paired: [browserClient],
    });
...
if [ "\${1:-}" = "devices" ] && [ "\${2:-}" = "list" ]; then
  ...
  else printf '%s\n' ${JSON.stringify(stickyLate)}; fi
  exit 0
fi
if [ "\${1:-}" = "devices" ] && [ "\${2:-}" = "approve" ]; then
  echo "$3" >> ${JSON.stringify(approveLog)}
  if [ "$3" = "browser-pair" ]; then
    printf '{}\n'
    exit 0
  fi
  echo "approve permanently failed" >&2
  exit 7
fi
echo "unexpected: $*" >&2
exit 2
`,
    ...
    try {
      ...
      expect(run.stdout).toContain("[auto-pair] approve failed request=sticky-cli:");
      // sticky-cli never increments APPROVED, so it never appears in an
      // "approved request=" line.
      expect(run.stdout).not.toMatch(/approved request=sticky-cli/);
      // Exactly one slow-mode marker: the rising-edge bump on sticky-cli.
      const slowMarkerRe = /fast-reentry bumped polls=3 approved=1 mode=slow/g;
      expect(run.stdout.match(slowMarkerRe)?.length).toBe(1);
      // The watcher kept retrying sticky-cli across multiple polls
      // (transient/sticky failures stay retryable until DEADLINE), proving
      // the override exhausted without resetting on every retry attempt.
      const attempts = fs.readFileSync(approveLog, "utf-8").trim().split("\n");
      const stickyAttempts = attempts.filter((id) => id === "sticky-cli").length;
      expect(stickyAttempts).toBeGreaterThanOrEqual(2);
      expect(run.stdout).toContain("watcher deadline reached approvals=1");
    } finally {
      fs.rmSync(tmpDir, { recursive: true, force: true });
    }
