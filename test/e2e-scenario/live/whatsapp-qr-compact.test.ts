...
const runWhatsappQrCompactTest = shouldRunLiveE2EScenarios() ? test : test.skip;
...
runWhatsappQrCompactTest(
  "WhatsApp pairing QR renders compact with the NemoClaw preload",
  {
    timeout: INSTALL_TIMEOUT_MS + PROBE_TIMEOUT_MS * 2,
  },
  async ({ artifacts, host }) => {
    await artifacts.writeJson("scenario.json", {
      id: "whatsapp-qr-compact",
      runner: "vitest",
      boundary: "upstream-openclaw-whatsapp-renderer",
      migratedFrom: "test/e2e/test-whatsapp-qr-compact-e2e.sh",
    });
    ...
    await artifacts.writeJson("versions.json", { openclawVersion });
    ...
    try {
      ...
      const install = await host.command(
        ...
        {
          artifactName: "whatsapp-qr-compact-install",
          cwd: workdir,
          inheritEnv: true,
      ...
      const baseline = await host.command("node", ["probe.mjs"], {
        artifactName: "whatsapp-qr-compact-baseline",
        cwd: workdir,
        inheritEnv: true,
        timeoutMs: PROBE_TIMEOUT_MS,
      });
      expect(baseline.exitCode, `baseline probe failed\n${baseline.stderr}`).toBe(0);
      const baselineDimensions = parseDimensions(baseline.stdout, "baseline");
      await artifacts.writeJson("baseline-dimensions.json", baselineDimensions);
      expect(
      ...
      const patched = await host.command("node", ["probe.mjs"], {
        artifactName: "whatsapp-qr-compact-patched",
        cwd: workdir,
        inheritEnv: true,
        env: {
      ...
      expect(patched.exitCode, `patched probe failed\n${patched.stderr}`).toBe(0);
      const patchedDimensions = parseDimensions(patched.stdout, "patched");
      await artifacts.writeJson("patched-dimensions.json", patchedDimensions);
      expect(
