...
The onboarding wizard lists **Hermes Provider** as a single inference-menu entry by design; it is not split into one entry per upstream model vendor.
Selecting it routes Hermes Agent through the host OpenShell provider that NemoClaw registers against Nous Research, and the individual vendor families it serves (Moonshot, Xiaomi, Tencent, Anthropic, OpenAI, Google, Qwen, StepFun, MiniMax, Z-AI, xAI, NVIDIA, and Arcee) are surfaced at the **model** selection step that follows, not as separate provider-menu items.
</AgentOnly>
...
| Hermes Provider | Routes Hermes Agent through the host OpenShell provider registered by NemoClaw when onboarding Hermes Agent. This is a single menu entry by design; the upstream model vendors it serves appear at the model selection step, not as separate provider-menu items. | Curated Hermes Provider models spanning every supported vendor family: Moonshot (`moonshotai/kimi-k2.6`), Xiaomi (`xiaomi/mimo-v2.5-pro`), Tencent (`tencent/hy3-preview`), Anthropic (`anthropic/claude-opus-4.7`), OpenAI (`openai/gpt-5.5`), Google (`google/gemini-3-pro-preview`), Qwen (`qwen/qwen3.5-plus-02-15`), StepFun (`stepfun/step-3.5-flash`), MiniMax (`minimax/minimax-m2.7`), Z-AI (`z-ai/glm-5.1`), xAI (`x-ai/grok-4.20-beta`), NVIDIA (`nvidia/nemotron-3-super-120b-a12b`), and Arcee (`arcee-ai/trinity-large-thinking`). |
