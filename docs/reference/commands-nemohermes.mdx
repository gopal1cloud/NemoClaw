...
Per-agent `model`, `subagents.*`, top-level `defaults`, and `main` overrides need a sandbox rebuild and are surfaced as warnings rather than silently dropped; rerun `nemohermes onboard --agents <agents.yaml> --recreate-sandbox` to bake those fields.
