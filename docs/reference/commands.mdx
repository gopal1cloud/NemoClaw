...
The command is a thin pass-through to `openclaw agent ...` inside the sandbox via `openshell sandbox exec`, with `HOME=/sandbox` so the addressed agent profile resolves the same way as `connect`.
...
The exit code is the remote command's exit code, so host-side pipelines can branch on it. Streaming follows whatever `openclaw agent` already emits on stdout; no additional buffering is added.
...
`agent` is currently supported only on OpenClaw sandboxes. Hermes already exposes an OpenAI-compatible HTTP API on port `8642` inside the sandbox, so non-interactive use does not need a wrapper command.
