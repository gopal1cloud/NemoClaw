...
| LangChain Deep Agents Code | `agents/langchain-deepagents-code/manifest.yaml` | Declares the LangChain Deep Agents Code (`dcode`) harness contract — config directory `/sandbox/.deepagents`, env-managed provider/model selection (`--model <provider>:<model>`, `NVIDIA_API_KEY` and peers), and the `dcode` TUI entry point — so `nemoclaw onboard --agent langchain-deepagents-code` selects and persists the harness. The sandbox image, start script, policy additions, and build-time provider/model config generation are tracked follow-ups under [issue #4861](https://github.com/NVIDIA/NemoClaw/issues/4861). |
